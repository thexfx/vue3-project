...
import { createIdempotencyKey, deposit, getStoredUser } from '@/api/bank'
...
async function handleSubmit() {
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      error.value = response.message || '存款失败'
    }
  } finally {
      ...
      <div v-else class="info-card">
        <form class="form" @submit.prevent="handleSubmit">
          <label class="form-item">
            <span>金额</span>
            <input v-model="amount" type="number" min="0.01" step="0.01" placeholder="请输入存款金额" />
          </label>
          <label class="form-item">
            <span>备注</span>
